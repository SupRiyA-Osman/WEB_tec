...
{
    $conn = db_conn();
    $selectQuery = "INSERT into tickets (location_from, location_to, date, time, price, transportType, bookedBy, b_id) VALUES (:busTo, :busFrom, :date , :time, :price, :tType, :bookedBy, :busId)";
...
{
    $conn = db_conn();
    $selectQuery = "UPDATE tickets set location_from = ?, location_to = ?, date= ?, time = ?, price = ?, b_id = ? where ticket_id = ?";
    try {
        $stmt = $conn->prepare($selectQuery);
        $stmt->execute([
