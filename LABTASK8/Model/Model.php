...
function updateBusTicket($data, $ticketId)
{
    ...
    try {
        $stmt = $conn->prepare($selectQuery);
        $stmt->execute([
            test_input($data['busTo']),  test_input($data['busFrom']),test_input($data['date']), test_input($data['time']), test_input($data['price']),test_input($data['busId']),  test_input($data['ticketId'])
        ]);
    } catch (PDOException $e) {
        echo $e->getMessage();
    }
